...
It's same as [EnvironmentPlugin](https://rspack.rs/plugins/webpack/environment-plugin) but instead `process.env.{key}` it defines `import.meta.env.{key}`.
...
```ts title="src/typings/import-meta.d.ts"
