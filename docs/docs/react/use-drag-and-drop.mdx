...
This example uses state for style but you can set position to style directly
to element in event callbacks using ref or `event.target`.
...
By default observer uses same event object between callbacks, just change property values.
You can disable this behavior by passing option `reuseEvent: false`.
