...
This example will work but this is very inconvenient:
you will have to import this particular container everywhere where you want to get a component from it. So, it more looks like "Service locator", but not "DI container"

Service locators is almost like DI containers, but with one problem - they exist as global variables in your project. Generally, this is not always a bad pattern.
For example, in game development, this is a common approach that works quite well.
...
What if we use an object instead of a primitive? We can easily attach a specific type to objects in TypeScript.
